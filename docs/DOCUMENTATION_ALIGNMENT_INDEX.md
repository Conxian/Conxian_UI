# Documentation Alignment Index

This index tracks the alignment of Conxian documentation across the repository and external surfaces.

## Core Documentation
- [AGENTS.md](../AGENTS.md) - Agent operating instructions and UI standards.
- [ARCHITECTURE.md](../ARCHITECTURE.md) - High-level system architecture and data flows.
- [README.md](../README.md) - Project entry point and setup instructions.
- [REPO_OWNERSHIP.md](../REPO_OWNERSHIP.md) - Repository ownership matrix, cross-repo mapping, and legacy deprecation rules.
- [CONTRIBUTING.md](../CONTRIBUTING.md) - Contribution guidelines.

## Standards and Matrices
- [FRONTEND_PRD.md](FRONTEND_PRD.md) - Global UI/UX standardization and Bright Foundation PRD.
- [UI_NAMING_STANDARDIZATION_MATRIX.md](UI_NAMING_STANDARDIZATION_MATRIX.md) - UI terminology standards.

## Research and OpenSpec
- [OpenSpec Specs](../openspec/specs/) - Specification-driven design docs.
- [Market Integration Spec](../openspec/specs/market-integration/spec.md) - Conxian Market API, ERP settlements, and treasury runway telemetry specification.
- [Remediation Research (July 2026)](../openspec/changes/2026-06-12-remediation-and-expanded-research/research_findings.md) - Cross-chain and institutional research, containing full multi-cloud alignment audits for Neon, Supabase, and Render.

## External Docs
- [Conxian Labs](https://conxian-labs.com) - Portfolio and ecosystem directory.
- [Conxian Protocol](https://conxian.org) - Primary brand and product surface.
